...
def validate(input_value, default_value, input_type, predicate, error_message):
    ...
    Args:
        ...
        input_type: Type or constructor taking the input value
        predicate: A unary truth-value function which is run against the
    output of the type / constructor applied to the input value.
        error_message: Error message to log on validation failure. The
    message may contain "{value}" to refer to the input value.

    Returns:
        The converted input value if validation is successful, otherwise
