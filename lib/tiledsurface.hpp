...
public:
  TiledSurface(PyObject * self_) {
      c_surface = mypaint_tiled_surface_new(self_);
  }
  ...
  void set_symmetry_state(bool active, float center_x) {
    mypaint_tiled_surface_set_symmetry_state(c_surface, active, center_x);
  }

  virtual void begin_atomic() {
      mypaint_tiled_surface_begin_atomic(c_surface);
  }
  virtual void end_atomic() {
      mypaint_tiled_surface_end_atomic(c_surface);
  }

  virtual uint16_t * get_tile_memory(int tx, int ty, bool readonly) {
      return mypaint_tiled_surface_get_tile(c_surface, tx, ty, readonly);
  }
